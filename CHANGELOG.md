...
<!-- Здесь накапливаются изменения для следующего milestone. -->
...
- **Ломающее (формат ключей):** инъективная escape-кодировка составных ключей
  ...
  в компонентах, поэтому `:` внутри `address`/`external_id`/`target.address`
  (реально — Matrix room-id `!room:server`, event-id `$id:server`) больше не
  схлопывает разные источники/сообщения/адреса доставки в один ключ. Раньше
  простой colon-concat был не инъективен. Для компонентов без `:`/`\`
  (числовые Telegram chat-id) кодировка байт-в-байт прежняя — Telegram-
  персистентность не затронута; меняются только Matrix-ключи (ровно
  багованные). Схема БД не меняется. Чистый разрыв без data-миграции
  (pre-alpha, релиза с Matrix не было; старый формат неоднозначен →
  точная миграция невозможна): осиротевшие Matrix-курсоры регенерируются
  одним catch-up-проходом. ADR 2026-06-20.
...
- Per-pipeline пересылка медиа (T033): флаг `[pipelines.*].forward_media`
  ...
  один источник может питать пайплайн-зеркало с медиа и текст-только
  пайплайн одновременно. Скачивание медиа остаётся отдельной глобальной
  `[media]`-политикой. ADR 2026-06-18.
...
- Пример медиа `examples/media/` (M7/A4, T034): зеркало одним аккаунтом A→B с
  пересылкой вложений + кастомный процессор `media_note`, читающий `local_path`
  скачанного файла. Показывает медиа-конвейер end-to-end (скачивание в
  git-ignored `angarion-data/media/`, чтение пути процессором). Живой прогон —
  документ из A доехал в B с аннотацией подписи.
...
- Per-write ретрай на `database is locked` в SQLite-сторах (T028, backstop
  к ADR §3.1/T024): в раздельном режиме (`--role api` + `--role pipeline`)
  два процесса пишут в один `app.db`; под WAL писатели сериализуются на
  едином write-lock'е, и при исчерпании `busy_timeout` (всплеск админ-
  операций, медленный диск) SQLite отдавал `database is locked` — запись
  падала. Все write-методы обёрнуты декоратором `_retry_on_locked`
  (tenacity, 5 попыток с backoff, `reraise=True`); ретраится **ровно**
  `database is locked`, не любой `OperationalError`. Транзакции —
  идемпотентные короткие `session.begin()`, безопасные к полному повтору.
  Поведение прод-движка не изменилось (`busy_timeout` прежний); `make_engine`
  получил тест-параметр `busy_timeout_ms`. ADR 2026-06-19.
...
- Флак e2e kill-теста `test_deterministic_kill_point_keeps_delivery[before_ack]`
  ...
  outbox, а доставкой занят отдельный конкурентный `DeliveryWorker`. Его окно
  `send→mark_sent` (§7.1) может совпасть с kill и дать **легитимный** дубль
  мишени — ни потеря, ни нарушение контракта. Тест ждал «ровно одну» доставку
  и перемежающе падал `2 == 1` под нагрузкой CI (на 3.13/3.14 и локально без
  нагрузки гонка не проявлялась). Теперь ждём `>= 1`; верхнюю границу (≤ 1
  дубль на kill) по-прежнему держит `_assert_dup_budget`. Продакшен-код не
  менялся — поведение at-least-once корректно, переусердствовал тест.
