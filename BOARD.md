...
- **T034** — Пример медиа (M7/A4, user-facing часть среза A3): `examples/media/`
  — зеркало **одним** аккаунтом в двух группах с `[media] download=true` +
  кастомный процессор, читающий `local_path` скачанного вложения (целевой
  сценарий спеки «доступ процессора к контенту»). Отдельная ветка/PR
  `T034-...` (по договорённости «пример — отдельная задача», как T018 для M3).
  Acceptance: на реальном аккаунте сообщение с фото/видео/документом из
  группы A доходит в B; включённое скачивание кладёт файл в git-ignored
  `angarion-data/media/`, процессор видит `local_path` (лог размера/пути или
  добавление подписи); `data/`/сессии/`media/` git-ignored, в файлах примера
  только плейсхолдеры; README со сценарием и пометкой про ретеншн.
