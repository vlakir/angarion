...
- **T025** — **M5/D** — Viz + Docs (ветка `T025-viz-docs`). [closed
  2026-06-15, PR #19] Последняя группа M5. Сделано: (1) `/ui/pipelines` —
  трёхдольный серверный SVG-граф «источники → пайплайны → получатели»
  (Jinja, без JS-библиотек), цвет узла = статус (активен / пауза из
  `runtime_config` / failed за час), аннотации delivered/depth, клик
  (admin) → htmx pause/resume; (2) **документация §17.9** (втянут
  mkdocs-каркас **T006** — закрыт этой задачей): mkdocs-material +
  mkdocstrings-автодок + быстрый старт + три гайда (deploy /
  plugin-authoring / web-API); (3) `examples/web/` — combined-пример с
  кастомной ручкой/страницей через DI поверх портов. Заодно: dev-запуск
  примеров без ручной возни (`.secrets` + засев сессии); **английский
  Web UI** (i18n веб-морды). Всплывший gap раннера (нет seam'а для
  user-страниц в `--with-api`) запаркован **T029**. Acceptance закрыт:
  офлайн UI, server-side граф, pause через `/ui/pipelines`, mkdocs
  `--strict` зелёный, пример воспроизводим. Закрывает milestone M5 →
  релиз **0.3.0** (без мажорного, решение Владимира 2026-06-15).
