...
командный outbox; consumer (pipeline-процесс) отправляет сообщение через
``MessageSinkPort`` (§12.9). Постановка **неблокирующая**: сбой (нет
...
``chat_id``); пустая — уведомление выключено (no-op).
...
def build_registration_message(deps: AngarionDeps, login: str) -> OutboundMessage:
    """
    Собрать ``OutboundMessage`` уведомления о заявке по ``[api.notify]``.

    ``account`` резолвится в ``[accounts.*]`` за messenger'ом; неизвестный
    ...
    return OutboundMessage(
        ...
        target=Address(
            messenger=messenger, chat_id=notify.chat_id, thread_id=notify.thread_id
        ),
        send_via=AccountRef(messenger=messenger, account_id=notify.account),
...
async def notify_registration(deps: AngarionDeps, *, login: str) -> None:
    ...
    if not deps.settings.api.notify.enabled:
        return
    try:
        message = build_registration_message(deps, login)
