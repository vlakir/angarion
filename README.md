...
> **Статус: pre-alpha — текущий релиз [0.4.0](CHANGELOG.md) (2026-06-20).**
> Реализован первый класс транспорта — **мессенджеры** (Telegram,
> Matrix/E2EE): дорожная карта M1–M7 пройдена (ядро, персистентность,
> LLM/`template`-процессоры, Web API/UI + auth, интеграционный контур).
> **Видение смещается к универсальному мультиформатному движку**, и его
> фундамент уже начал приземляться: транспорт-агностичная модель
> `Record`/`transport` (T041), цепочки пайплайнов через внутренний провод
> (T037), ручной/веб-триггер пайплайна (T038) — всё в 0.4.0. Впереди —
> новые классы транспорта (брокеры T039, email T040); см.
