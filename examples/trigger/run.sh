#!/usr/bin/env bash
# Пример angarion «trigger»: ручной запуск пайплайна (T038). Passthrough A→B,
# но обработку инициирует не живое сообщение, а ручной триггер.
#
# Два режима запуска:
#   examples/trigger/run.sh           # combined-сервер (--with-api): HTTP + UI
#   examples/trigger/run.sh inject    # программный впрыск (inject.py), затем выход
#
...
# API-ключ ручного триггера. Demo-значение для удобства прогона; в бою задай
# свой секрет (env приоритетнее). С пустым токеном HTTP-ручки выключены (503).
export ANGARION_API__TRIGGER_TOKEN="${ANGARION_API__TRIGGER_TOKEN:-demo-trigger-token}"
...
# 4. запуск выбранного режима
case "${1:-serve}" in
  serve)
    echo "→ Combined-сервер. Web UI: http://127.0.0.1:8000/ui"
    echo "  Кнопка триггера: http://127.0.0.1:8000/ui/trigger"
    echo "  HTTP-ручка (API-ключ из \$ANGARION_API__TRIGGER_TOKEN) — примеры curl в README.md."
    echo "  Ctrl+C — стоп."
    uv run angarion run --config "$CONFIG" --with-api
    ;;
  inject)
    echo "→ Программный впрыск (inject.py): submit_event + run_pipeline → доставка в B."
    uv run python inject.py
    ;;
  *)
    echo "usage: run.sh [serve|inject]" >&2
    exit 2
    ;;
esac
