...
**angarion** — универсальная Python-библиотека конвейеров обработки
событий сообщений (новое / отредактировано / удалено): N источников →
нормализация → очередь → обработка → M получателей, с аналитикой в БД.
Гексагональная архитектура (ports & adapters). Первый целевой адаптер —
Telegram (Telethon, MTProto, пользовательские аккаунты). Полное ТЗ —
`CONCEPT.md` (v2.1); этапы реализации M1–M7 — §16 ТЗ.

Проект-специфичные требования качества (из ТЗ, жёстче дефолтов
шаблона): **TDD обязателен** (тест пишется до кода), **coverage ≥ 90%**
(`--cov-fail-under=90`), `mypy --strict` на пакет; `migrations/`
(Alembic) исключён из линтеров, mypy и coverage. Минимальная версия
Python — **3.12** (библиотека, широкая совместимость).
...
- Python 3.12+ (`requires-python` в `pyproject.toml`; в этом проекте
  опущено с шаблонных 3.14 до 3.12 — решение от 2026-06-11, библиотека).
...
- Тестовый стек: `pytest` + `pytest-cov` + `pytest-asyncio`. Coverage
  threshold в этом проекте — **≥ 90%** на `src/` (`--cov-fail-under=90`
  в `[tool.pytest.ini_options]`; требование ТЗ, выше шаблонных 80).
...
### Гигиена репозитория: мусор и секреты (требование Разработчика, 2026-06-11)

Перед каждым коммитом проверять `git status` на предмет случайных
артефактов. В репозиторий **никогда** не попадают:

- секреты: `.env`, `api_id`/`api_hash`, JWT-секреты, пароли;
- session-файлы Telethon (`*.session`, `sessions/`) — это полноценные
  учётные данные аккаунта;
- локальные данные рантайма: `data/`, `*.db` (app.db, queue.db);
- IDE-артефакты (`.idea/` и т.п.), кэши инструментов, временные файлы.

`.gitignore` уже покрывает перечисленное; при появлении нового вида
артефактов — сначала дополнить `.gitignore`, потом коммитить. Если
секрет всё же попал в индекс или историю — немедленно сообщить
Разработчику, а не «тихо удалить следующим коммитом».

### Примеры как часть крупной фичи (договорённость 2026-06-13)

После реализации крупной фичи (со `specs/T<NNN>-*/spec.md`), **с которой
конечный пользователь библиотеки взаимодействует напрямую** — новый
адаптер платформы, процессор, способ конфигурации/запуска, UI — завести
один или более примеров в `examples/` и пройтись по существующим на
актуальность. Это часть deliverable этапа (§17.9 ТЗ: документация не
послесловие), а заодно примеры выявляют реальные баги (на демо M3 так
всплыл баг резолва по числовому id) и держат quick start воспроизводимым.

**Критерий «нужен ли пример» — есть ли что показать в сценарии «как
этим пользоваться».** Инфраструктурные слои (персистентность/хранилище/
очередь, внутренние порты, тестовые контракты `angarion.testing`,
рефакторинг) примера **не требуют**, даже имея спеку: пользователь не
вызывает их напрямую, а проявляются они через строчку конфига
(`backend = "..."`) уже внутри user-facing примеров. Сверка по этапам:
M3/M4/M5/M7 — пример нужен; M1/M2/M6 — нет.

Как применять:

- Включать «примеры» в шаг **Update** workflow крупных фич (рядом с
  `CHANGELOG.md` / `DECISIONS.md`).
- Пример заводить **отдельной задачей** `T<NNN>` (как T018 для M3 —
  `examples/forward/`), со своей веткой/PR.
- При изменении публичного API/конфига существующих фич — проверить
  `examples/` и обновить устаревшее (актуальность — обязательна).
- Секреты и рантайм примеров — git-ignored; в файлах примера только
  плейсхолдеры (см. «Гигиена репозитория»).
- Мелкие правки/багфиксы новых примеров не требуют.
