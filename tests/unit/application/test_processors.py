...
class TestRegistry:
    ...
    async def test_function_processor_delegates(self) -> None:
        ...
        result = await proc.process(make_record(), make_context(), make_services())
...
class TestPassthrough:
    async def test_delivers_to_all_targets(self) -> None:
        ...
        assert [msg.target.address for msg in result.outbound] == [
            '-100111',
            '-100222',
        ]
        expected_keys = [
    ...
    async def test_media_forwarded_to_all_targets(self) -> None:
        ...
        OutboundRecord (sender научится их слать в A2).
        """
        proc = get_processor('passthrough')
        media = [MediaRef(kind='photo', ref='file42')]
        event = make_record(media=media)
        targets = [make_target('-100111'), make_target('-100222')]
        ctx = make_context(targets=targets)
        result = await proc.process(event, ctx, make_services())
        assert result.verdict is Verdict.DELIVER
    ...
    async def test_no_media_yields_empty_outbound_media(self) -> None:
        """Без вложений OutboundRecord.media пуст (а не None)."""
        proc = get_processor('passthrough')
        result = await proc.process(make_record(), make_context(), make_services())
    ...
    async def test_media_only_delivered_with_empty_caption(self) -> None:
        ...
        event = make_record(text=None, media=media)
        result = await proc.process(event, make_context(), make_services())
        assert result.verdict is Verdict.DELIVER
    ...
    async def test_no_text_no_media_dropped(self) -> None:
        ...
        proc = get_processor('passthrough')
        event = make_record(kind=RecordKind.DELETED, text=None, media=[])
        result = await proc.process(event, make_context(), make_services())
        assert result.verdict is Verdict.DROP
...
class TestTemplate:
    ...
    async def test_delivers_rendered_text_to_all_targets(self) -> None:
        """§10.2: рендер базового шаблона → OutboundRecord по ключу на цель."""
        proc = get_processor('template')
        event = make_record(text='мир')
        targets = [make_target('-100111'), make_target('-100222')]
        ctx = make_context(
    ...
    async def test_edited_falls_back_to_base_template(self) -> None:
        proc = get_processor('template')
        event = make_record(kind=RecordKind.EDITED, text='t')
        ctx = make_context(
    ...
    async def test_deleted_uses_deleted_template(self) -> None:
        ...
        proc = get_processor('template')
        event = make_record(kind=RecordKind.DELETED, text=None, external_id='7')
        ctx = make_context(
            pipeline='tmpl_deleted',
            settings={
                'template': '{{ text }}',
                'deleted': 'удалено #{{ external_id }}',
            },
        )
    ...
    async def test_invalid_config_raises_config_error(self) -> None:
        ...
        with pytest.raises(ConfigError, match='processor_config'):
            await proc.process(make_record(), ctx, make_services('tmpl_invalid'))
    ...
    async def test_bad_template_syntax_raises_config_error(self) -> None:
        ...
        with pytest.raises(ConfigError, match='processor_config'):
            await proc.process(make_record(), ctx, make_services('tmpl_syntax'))
    ...
    async def test_config_cached_per_pipeline(self) -> None:
        ...
        first = await proc.process(make_record(text='a'), ctx, svc)
        second = await proc.process(make_record(text='b'), ctx, svc)
